    ...
    public class SampleConsole
    {
        private static async Task AuthorizeAsync(Config config)
        {
            try
            {
                ...
                // step 2
                var url = $"{connection.BaseUrlPrefix}/oauth/confirm_access?oauth_token={HttpUtility.UrlEncode(connection.RequestToken)}";
                Process.Start(url);
        ...
        {
            var config = SimpleJson.DeserializeObject<Config>(File.ReadAllText("config.json"));

            if (args.Contains("-a")) AuthorizeAsync(config).Wait();
            else TestAsync(config).Wait();
        }
        ...
        private static async Task TestAsync(Config config)
        {
            ...
            if (projects.RealEstateProject.Any())
            {
                ...
                if (!entries.RealEstateProjectEntry.Any(e => e.RealEstateId.Value == realEstate.RealEstate.Id.Value)) await api.RealEstateProjects.AddAsync(project.Id.Value, realEstate.RealEstate);
            }
